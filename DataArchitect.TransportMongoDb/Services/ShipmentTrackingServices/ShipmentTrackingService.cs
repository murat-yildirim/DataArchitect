...
namespace DataArchitect.TransportMongoDb.Services.ShipmentTrackingServices
{
    public class ShipmentTrackingService : IShipmentTrackingService
    {
        ...
        public async Task<List<ResultShipmentTrackingDto>> GetAllTrackingAsync(string trackingNumber)
        {
            var shipment = await _shipmentCollection.Find(x => x.TrackingNumber == trackingNumber).FirstOrDefaultAsync();
        ...
        {
            var shipment = await _shipmentCollection.Find(x => x.TrackingNumber == trackingNumber)
                .FirstOrDefaultAsync();
